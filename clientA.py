...
# Dictionaries to store certificates and public keys of other clients, and key fragments
client_certificates = {}
others_public_keys = {}
Key_frgments = {}
...
# Encode client identifiers
clientB = 'B'.encode('utf-8')
clientC = 'C'.encode('utf-8')
current_client = 'A'.encode('utf-8')

# Generate certificate, signature, and keys for client A
cert_pem, signature, private_key, public_key = Public.generate_cert('A')

def response_with_Kabc(sock):
    """Function to respond with the combined session key Kabc."""
    print("A send response with Kabc")
    sock.sendall('send response to others'.encode('utf-8'))

    # Encrypt the session key with the public keys of B and C
    encrypted_Pass1 = Public.encrypt_data_with_aes(combinedKabc, Key_frgments['B'])
    encrypted_Pass2 = Public.encrypt_data_with_aes(combinedKabc, Key_frgments['C'])
    ...
    response1 = clientB + separator + encrypted_Pass1 + separator + sign1
    response2 = clientC + separator + encrypted_Pass2 + separator + sign2
...
def wait_for_response(sock):
    ...
    count = 0
    while True:
        recieve_data = sock.recv(4096)
        if recieve_data != b'ACK':
            print("start to receive response")

            # Split the received data to extract the signature and the data
            parts = recieve_data.split(separator)
            data = separator.join(parts[:-1])
            sign = parts[-1]
            # Verify the signature
            if Public.verify_signature(others_public_keys['S'], sign, data) == True:
                ...
                if Public.verify_signature(others_public_keys[client_name], sign, encrypted_pass) == True:
                   if Key_frgments['A'] == decrypted_pass:
...
def send_pass_to_others(sock):
    ...
    print("send PassA to S")
    sock.sendall('send key fragment to others'.encode('utf-8'))
    if sock.recv(1024) == b'ACK':
        # Generate and send PassA
        PassA = Public.generate_key_fragment()  # Logic to generate PassA goes here
        Key_frgments['A'] = PassA
        sign = Public.sign_data(private_key, PassA)  # Sign PassA with A's private key
        # Encrypt PassA for B and C
        PassA_to_B = Public.encrypt_data(others_public_keys['B'], PassA + separator + sign)  # Encrypt PassA for B
        PassA_to_C = Public.encrypt_data(others_public_keys['C'], PassA + separator + sign)  # Encrypt PassA for C
        Data_to_S = Public.encrypt_data_20(others_public_keys['S'], clientB + separator + PassA_to_B + separator + clientC + separator + PassA_to_C)
        sign = Public.sign_data(private_key, Data_to_S)
    ...
    while True:
        ...
        if recieve_data != b'ACK':
            ...
            if Public.verify_signature(others_public_keys['S'], sign, data) == True:
                ...
                client_name = client_name.decode('utf-8')
                if Public.verify_signature(others_public_keys[client_name], sign, decrypted_pass) == True:
            ...
            if len(Key_frgments) == 3:
                ...
                print(f"Combined key is {combinedKabc}")
                break

def send_request_for_certificates(sock):
    """Request certificates for clients B and C from server S."""
    print("request CertB, CertC")
...
def send_messages_to_server():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        if Public.Authentication(data):
            ...
            while True:
                print("\nSelect an option:")
                print("1. Request B and C's certificates and signatures from server S.")
                print("2. exchange key fragment through S.") #challenge
                print("3. response with Kabc.") #response
...
def send_encrypted_messages(sock):
    ...
    while True:
        ...
        print("Encrypted message sent to B and C.")
