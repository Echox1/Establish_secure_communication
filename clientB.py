...
current_client = 'B'.encode('utf-8')

# Variable to store the combined session key
combinedKabc = bytes()

# Generate certificate, signature, and keys for client B
cert_pem, signature, private_key, public_key = Public.generate_cert('B')

def response_with_Kabc(sock):
    """Function to respond with the combined session key Kabc."""
    print("B send response with Kabc")
    sock.sendall('send response to others'.encode('utf-8'))

    # Encrypt the session key with the public keys of A and C
        ...
        if receive_data != b'ACK':
            ...
            if Public.verify_signature(others_public_keys['S'], sign, data):
                ...
                if Public.verify_signature(others_public_keys[client_name], sign, encrypted_pass):
                    if Key_frgments['B'] == decrypted_pass:
...
def send_pass_to_others(sock):
    ...
    print("send PassB to S")
    sock.sendall('send key fragment to others'.encode('utf-8'))
    if sock.recv(1024) == b'ACK':
        # Generate and send PassB
        PassB = Public.generate_key_fragment()  # This should have the logic to generate PassB
        Key_frgments['B'] = PassB

        sign = Public.sign_data(private_key, PassB)  # Sign PassB with B's private key
        # Encrypt PassB for A and C
        PassB_to_A = Public.encrypt_data(others_public_keys['A'], PassB + separator + sign)  # Encrypt PassB for A
        PassB_to_C = Public.encrypt_data(others_public_keys['C'], PassB + separator + sign)  # Encrypt PassB for C
        Data_to_S = Public.encrypt_data_20(others_public_keys['S'], clientA + separator + PassB_to_A + separator + clientC + separator + PassB_to_C)
        sign = Public.sign_data(private_key, Data_to_S)
        sock.sendall(sign + separator + Data_to_S)
...
def send_request_for_certificates(sock):
    """Request certificates for clients A and C from server S."""
    print("request CertA, CertC")
...
def send_messages_to_server():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        if Public.Authentication(data):
            ...
            while True:
                print("\nSelect an option:")
                print("1. Request A and C's certificates and signatures from server S.")
...
def send_encrypted_messages(sock):
    ...
    while True:
        ...
        print("Encrypted message sent to A and C.")
